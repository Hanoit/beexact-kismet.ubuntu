...
class TestRepositoryImpl(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.vendor_repository = RepositoryImpl(MACVendorTable, self.Session)

    def test_search_by_id(self):
        # Prepare the mock session to return a specific vendor
        mac_address = '00-00-05'
        ...
        vendor_name = self.vendor_repository.search_by_id(mac_address).vendor_name

        # Verify the results
        self.assertEqual(vendor_name, expected_vendor)

        # Verify that the query method was called correctly
        mock_session.query.assert_called_with(MACVendorTable)
